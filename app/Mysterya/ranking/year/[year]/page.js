...
export default async function Page({ params, searchParams }) {
	const url = process.env.BASE_URL + encodeURI(`/api/mysterya/ranking/year/${params.year}`);
	...
	const BF_king = Object.values(sorted).sort(
		(a, b) =>
			...
			<div className={styles.submain}>
									...
									{ZT_king.map((obj, index) => {
										return (
											<tr key={index}>
												...
												<td>{obj.ZT.BB}</td>
												<td>{obj.ZT.H}</td>
												<td>{(obj.ZT.SS + obj.ZT.O) / (obj.ZT.BB + obj.ZT.H ? obj.ZT.BB + obj.ZT.H : 1)}</td>
											</tr>
										);
									})}
								</tbody>
				...
				<div>
					<div className={`${styles.cards} ${styles.LR}`}>
						...
						<Card
							name={sorted[Object.keys(BF_king[0].BF)[0]].name}
							number={Object.keys(BF_king[0].BF)[0]}
							no={0}
							data1={['불러들인 횟수', Object.values(BF_king[0].BF)[0]]}
						/>
					</div>
					<details>
						<summary className={styles.summary}>전체보기</summary>
